...
# GRASP + Person Tracking Integration

Launch only Grasp, without any person tracking:
```bash
ros2 launch path_tracking grasp_only.launch.py
```

Launch Grasp with RGB or thermal based person following:
```bash
ros2 launch path_tracking grasp_follow_rgb.launch.py
```

```bash
ros2 launch path_tracking grasp_follow_thermal.launch.py
```

Run either RGB or thermal based person tracking without GRASP. Here the drone will first climb to the `take_off_height` and then scan for people:

```bash
ros2 launch path_tracking follow_rgb_only.launch.py
```

```bash
ros2 launch path_tracking follow_thermal_only.launch.py
```

Then send the trigger service request for the grasp path planner:
```bash
ros2 service call plan std_srvs/srv/Trigger {}
```

Finally, arm the drone to take off.
...
- Clean up code
  - Make `.yaml` files for different cameras (intrinsics, detection topics, etc) and load via launch files. Remove the `rgb.` and `thermal.` parameters in `path_control.cpp`.
- Add detection launch files for different scenarios
  - USB cam + YOLO
  - FLir usb + fieldscan + Yolo
- Test RGB & Thermal scenarios in simulation	
