...
private:
  float estimateDistanceFromBBoxHeight(float bbox_height_pixels)
    {
        if (bbox_height_pixels <= 1.0f)
            return 50.0f;  // very far

        float _distance_calib_k = 400.0f;  // Calibration constant (tuned for your camera and target)
        return _distance_calib_k / bbox_height_pixels;
    }
  ...
  void loadParameters()
    ...
    void detectionCallback(const yolo_msgs::msg::DetectionArray::SharedPtr data)
    {
        ...
        // https://github.com/HanaanMakhdoomi/Person-Following-Drone-RASPBERRY-PI/blob/main/PersonFollowingDrone.py

        float vbx = _dist_kp * (_desired_size - size_y) / _desired_size;

        float vbz = _height_kp * (_center_y - y) / _center_y;

        RCLCPP_INFO_THROTTLE(_node.get_logger(), *_node.get_clock(), 500, "Target ID: %s | vbx: %.2f | x: %.1f y: %.1f | size_y: %.1f", id.c_str(), vbx, x, y, size_y);
        ...
        // FOR KEEPING THE TARGET CENTERED ------------(BOTH VERTICAL AND HORIZONTAL)
