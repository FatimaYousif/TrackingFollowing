...
private:
    void loadParameters()
    {
        _detection_topic        = get_parameter("subscribers.detection_topic").as_string();
        ...
        _center_x               = static_cast<float>(get_parameter("camera.center_x").as_double());
        _center_y               = static_cast<float>(get_parameter("camera.center_y").as_double());
        _desired_size           = static_cast<float>(get_parameter("camera.desired_target_size").as_double());
    ...
    {
        if (!_vehicle_status.flag_control_offboard_enabled && !_pilot_takeover) {
