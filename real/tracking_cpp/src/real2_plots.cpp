  ...
  void loadParameters()
    {
        ...
        // RCLCPP_INFO(_node.get_logger(), _detection_topic); 

        RCLCPP_INFO(_node.get_logger(), "detection topic: %s",_detection_topic );
    ...
    void publishAnalysis(const std::string &mode)
    {
        ...
        // controller error: Euclidean distance of pixel centroid error normalised to metres
        // using the existing pixel errors you already compute
        float px_err_x =  msg.pixel_error_x;                // _center_x - _last_target_x;
        float px_err_y = msg.pixel_error_y;               // _center_y - _last_target_y;
        // scale factor: treat image-plane error as a proxy; replace with your real metric if available
        msg.controller_error = std::sqrt(px_err_x * px_err_x + px_err_y * px_err_y)
                            / _center_x;   // normalised [0..~√2], multiply by scale if known
    ...
    const ultralytics_ros::msg::YoloResult::SharedPtr msg)
    ...
    // void detectionCallback(const yolo_msgs::msg::DetectionArray::SharedPtr data)
    {
        ...
        // target = &data->detections[0];
        // target = &msg->detections.detections[0];
        ...
        _has_detection = true;
        _no_detection_counter = 0;
        _detection_frame_count++;   // <-- increment only on valid detection
    ...
    int _detection_frame_count{0};    // frames with valid detection
    int _total_frame_count{0};        // all frames
