...
: Node("path_control")
{
  ...
  timer_ = create_wall_timer(
    ...
    // ----------------------gimbal
  //     gimbal_yaw_publisher_   = create_publisher<std_msgs::msg::Float64>(
  //   "/model/x500_gimbal_0/command/gimbal_yaw",   10);
  // gimbal_pitch_publisher_ = create_publisher<std_msgs::msg::Float64>(
  //   "/model/x500_gimbal_0/command/gimbal_pitch", 10);
}
...
void PathControl::declare_and_get_parameters()
{
  ...
  // ----------------------gimbal
  // Gimbal sweep params
  // this->declare_parameter<double>("gimbal.sweep_min",   -1.0);
  // this->declare_parameter<double>("gimbal.sweep_max",    1.0);
  // this->declare_parameter<double>("gimbal.sweep_rate",   0.3);
  // this->declare_parameter<double>("gimbal.pitch_angle", -0.3);

  // gimbal_sweep_min_   = this->get_parameter("gimbal.sweep_min").as_double();
  // gimbal_sweep_max_   = this->get_parameter("gimbal.sweep_max").as_double();
  // gimbal_sweep_rate_  = this->get_parameter("gimbal.sweep_rate").as_double();
  // gimbal_pitch_angle_ = this->get_parameter("gimbal.pitch_angle").as_double();
}


// ----------------------gimbal
// void PathControl::publish_gimbal(double yaw_rad, double pitch_rad)
// {
//   std_msgs::msg::Float64 y_msg, p_msg;
//   y_msg.data = yaw_rad;
//   p_msg.data = pitch_rad;
//   gimbal_yaw_publisher_->publish(y_msg);
//   gimbal_pitch_publisher_->publish(p_msg);
// }

// void PathControl::update_gimbal_sweep()
// {
//   // Advance angle by rate * dt  (dt = 1/frequency seconds)
//   const double dt = 1.0 / 100.0;   // matches your default frequency param
//   gimbal_yaw_angle_ += gimbal_sweep_dir_ * gimbal_sweep_rate_ * dt;

//   // Bounce at limits
//   if (gimbal_yaw_angle_ >= gimbal_sweep_max_) {
//     gimbal_yaw_angle_ = gimbal_sweep_max_;
//     gimbal_sweep_dir_ = -1;
//   } else if (gimbal_yaw_angle_ <= gimbal_sweep_min_) {
//     gimbal_yaw_angle_ = gimbal_sweep_min_;
//     gimbal_sweep_dir_ = +1;
//   }

//   publish_gimbal(gimbal_yaw_angle_, gimbal_pitch_angle_);
// }

// void PathControl::gimbal_center()
// {
//   gimbal_yaw_angle_ = 0.0;
