...
void PathControl::declare_and_get_parameters()
{
  ...
  this->declare_parameter<float>("following_yaw_kp", 0.5);
  this->declare_parameter<float>("following_dist_kp", 1.5);
  this->declare_parameter<float>("following_center_x", 960.0);
  this->declare_parameter<float>("following_center_y", 540.0);
  this->declare_parameter<float>("following_desired_size", 100.0);
  this->declare_parameter<int>("following_hover_threshold", 100);
  this->declare_parameter<std::string>("detection_topic", "/yolo/tracking");
  
  // Get following params
  following_yaw_kp_ = this->get_parameter("following_yaw_kp").as_double();
  following_dist_kp_ = this->get_parameter("following_dist_kp").as_double();
  following_center_x_ = this->get_parameter("following_center_x").as_double();
  following_center_y_ = this->get_parameter("following_center_y").as_double();
  following_desired_size_ = this->get_parameter("following_desired_size").as_double();
  following_hover_threshold_ = this->get_parameter("following_hover_threshold").as_int();
...
// ---------------------------------------------------------
// Add the detection callback implementation
void PathControl::detection_callback(const yolo_msgs::msg::DetectionArray::UniquePtr msg)
{
  ...
  has_replanned = false;  // <-- reset when target comes back
  
  // Only process if we're in MISSION state (or add a new FOLLOWING state if preferred)
  ...
  // Get current yaw from vehicle pose
  float current_yaw = drone_yaw_ros_;
  
  // Calculate control outputs
  float vbx = following_dist_kp_ * (following_desired_size_ - size_y) / following_desired_size_;
  float yaw_rate = -following_yaw_kp_ * (following_center_x_ - x) / following_center_x_;
  
  // Convert body velocity to world velocity
  following_velocity_x_ = vbx * std::cos(current_yaw);
  following_velocity_y_ = vbx * std::sin(current_yaw);
  following_yaw_rate_ = yaw_rate;
  
  RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 500,
    "Following target | vbx: %.2f | x: %.1f | size: %.1f | yaw_rate: %.2f",
    vbx, x, size_y, yaw_rate);
}

// Add helper functions
void PathControl::capture_hold_position()
{
  hold_position_x_ = vehicle_pose_ros_.pose.position.x;
  hold_position_y_ = vehicle_pose_ros_.pose.position.y;
  hold_altitude_ = vehicle_pose_ros_.pose.position.z;
}
...
void PathControl::handle_mission_state()
{
  ...
  // Check if we have a detection and should follow instead of path tracking
  if (has_detection_) {
    // Following mode - override path tracking with person following
    std::array<float, 3UL> position_array = {
      static_cast<float>(vehicle_pose_ros_.pose.position.x),
      static_cast<float>(vehicle_pose_ros_.pose.position.y),
      static_cast<float>(vehicle_pose_ros_.pose.position.z)
    };
    
    std::array<float, 3UL> velocity_array = {
      following_velocity_x_,
      following_velocity_y_,
      0.0f  // maintain altitude
    };
    
    // Publish with yaw rate for following
    publish_trajectory_setpoint_with_yawrate(position_array, velocity_array, following_yaw_rate_);
    
    RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000,
      "Following person - vel: (%.2f, %.2f) yaw_rate: %.2f",
      following_velocity_x_, following_velocity_y_, following_yaw_rate_);
  } 
  else if (no_detection_counter_ > 0 && no_detection_counter_ < following_hover_threshold_) {
    // Lost detection but within threshold - hover in place
    std::array<float, 3UL> hover_position = {
      hold_position_x_,
      hold_position_y_,
      hold_altitude_
    };
    std::array<float, 3UL> zero_velocity = {0.0f, 0.0f, 0.0f};
    publish_trajectory_setpoint(hover_position, drone_yaw_ros_, zero_velocity);
  ...
  // else {
  //   // No detection - continue with normal path tracking
  //   pure_pursuit_.setData(path_.poses, vehicle_pose_ros_.pose.position, vehicle_speed_, 0);
  //   auto control_output = pure_pursuit_.controlLoop();
  ...
  //   if (pure_pursuit_.isPathFinished(path_.poses)) {
  //     hover_position_ = std::array<float, 3UL>{
  ...
  // Target lost for too long → trigger replan ONCE
  if (no_detection_counter_ > following_hover_threshold_ && !has_replanned) {
    RCLCPP_WARN(get_logger(), "Target lost for too long → triggering replan");
    call_service();
    has_replanned = true;
  }

  // Continue with normal path tracking
  pure_pursuit_.setData(path_.poses, vehicle_pose_ros_.pose.position, vehicle_speed_, 0);
  auto control_output = pure_pursuit_.controlLoop();
  ...
  if (pure_pursuit_.isPathFinished(path_.poses)) {
    ...
    hover_velocity_ = {0.0, 0.0, 0.0};
    transitionToState(State::HOVER);
    pure_pursuit_.reset();
  }
}
  
  // Capture hold position when we lose detection
  if (!has_detection_ && no_detection_counter_ == 1) {
    capture_hold_position();
  }
}

// ---------------------------------
// Add new method for publishing with yaw rate
  ...
  float yaw_rate)
{
  ...
  // Create trajectory setpoint message with yaw_rate
  px4_msgs::msg::TrajectorySetpoint msg{};
  msg.position = position;
  msg.velocity = velocity;
  msg.yawspeed = yaw_rate;
  msg.yaw = NAN;  // Use yaw rate instead of absolute yaw
  msg.timestamp = get_clock()->now().nanoseconds() / 1000;
  trajectory_setpoint_publisher_->publish(msg);
}
