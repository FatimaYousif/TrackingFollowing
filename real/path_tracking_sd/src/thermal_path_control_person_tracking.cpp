#include "path_tracking/thermal_path_control_person_tracking.hpp"
#include <cmath>
...
: Node("path_control")
{
  ...
  // detection_subscriber_ = create_subscription<ultralytics_ros::msg::YoloResult>(
  //   detection_topic_,
  //   rclcpp::QoS(10).reliable(),
  //   std::bind(&PathControl::detection_callback, this, std::placeholders::_1));


  detection_subscriber_ = create_subscription<std_msgs::msg::Float32MultiArray>(
  "/yolo/bbox_info",
  rclcpp::QoS(10).reliable(),
  std::bind(&PathControl::detection_callback, this, std::placeholders::_1));
...
void PathControl::declare_and_get_parameters()
{
  ...
  this->declare_parameter<std::string>("thermal_subscribers.detection_topic", "/yolo/bbox_info");
  this->declare_parameter<int>("detection_loss.hover_frames_threshold", 1000);
  this->declare_parameter<double>("control.yaw_kp",            0.5);
  this->declare_parameter<double>("control.distance_kp",       0.8);
  this->declare_parameter<double>("thermal_camera.center_x",           0.0);   // from Benjamin's shared params -- cx and cy from calibration_0.3 exp on FLIR camera
  this->declare_parameter<double>("thermal_camera.center_y",           0.0);
  this->declare_parameter<double>("thermal_camera.desired_target_size", 0.0);       // ------------- to change for FLIR camera
  this->declare_parameter<double>("setpoint.search.yaw_rate",  0.3);

  detection_topic_           = this->get_parameter("thermal_subscribers.detection_topic").as_string();
...
// void PathControl::detection_callback(const ultralytics_ros::msg::YoloResult::SharedPtr msg)
...
void PathControl::detection_callback(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
  {
  if (msg->data.size() < 4)
  {
      has_detection_ = false;
  ...
  std::array<float, 3UL> velocity)
{
  ...
  // yaw = -drone_yaw_ros_;
  // yaw = -drone_yaw_ros_;                      // comment this  // for drone orientation during path tracking

  // for drone orientation during path tracking
