...
void PathControl::declare_and_get_parameters()
{
  ...
  this->declare_parameter<std::string>("rgb_subscribers.detection_topic", "/yolo_result");
  ...
  this->declare_parameter<double>("rgb_camera.desired_target_size", 292.0);       // ------------- to change for FLIR camera
  this->declare_parameter<double>("setpoint.search.yaw_rate",  0.3);

  detection_topic_           = this->get_parameter("rgb_subscribers.detection_topic").as_string();
