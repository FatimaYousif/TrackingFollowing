#include "path_tracking/path_control_person_tracking.hpp"
#include <cmath>
...
void PathControl::declare_and_get_parameters()
{
  ...
  this->declare_parameter<std::string>("subscribers.detection_topic", "/yolo_result");
  ...
  this->declare_parameter<double>("camera.center_x",           969.99036);   // cx and cy from calibration_0.3 exp on FLIR camera
  this->declare_parameter<double>("camera.center_y",           532.19128);
  this->declare_parameter<double>("camera.desired_target_size", 100.0);       // ------------- to change for FLIR camera
