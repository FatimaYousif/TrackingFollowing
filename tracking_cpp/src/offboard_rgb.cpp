...
private:
    void loadParameters()
    {
        _detection_topic        = get_parameter("rgb_subscribers.detection_topic").as_string();
        ...
        _center_x               = static_cast<float>(get_parameter("rgb_camera.center_x").as_double());
        _center_y               = static_cast<float>(get_parameter("rgb_camera.center_y").as_double());

        // RCLCPP_INFO(get_logger(), "testing rgb camera: detection topic=%s, center x=%.2f", _detection_topic.c_str(),_center_x);

        _desired_size           = static_cast<float>(get_parameter("rgb_camera.desired_target_size").as_double());
